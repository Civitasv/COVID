<template>
  <div style="height:100%;width:100%;">
    ...
    <div class="manage">
      ...
      <div class="check3">
        <Row graphType="填色图" :legend="fillLegend" @changeFillVisualData="changeFillVisualData"></Row>
      </div>
    </div>
  </div>
</template>
...
export default {
  data() {
    return {
      map: null,
      // 热力图
      sourceID: "china-virus-datas",
      heatID: "china-virus-heat",
      ...
      // 填色图
      fillID: "china-value",
      fillSourceID: "china-boundary",
      fillVisualData: "confirmed",
  ...
  methods: {
    ...
    changeFillVisualData(fillVisualData) {
      this.fillVisualData = fillVisualData;
      console.log(fillVisualData);
      if (this.fillVisualData == "") {
        // 如果是空，清除集合
        this.map.removeLayer(this.fillID);
        this.map.removeSource(this.fillSourceID);
      } else {
        if (this.map.getLayer(this.fillID)) this.map.removeLayer(this.fillID);
    ...
    ...mapActions("virus", ["getAllChinaVirus"]),
    async loadData() {
      // 发送请求获得数据
      if (this.allChinaVirus == "") await this.getAllChinaVirus();
      this.heatMap();
      this.fillMap();
      this.clusterMap();
      this.calData();
    ...
    fillMap() {
      ...
      this.map.addLayer({
        ...
        paint: {
          "fill-color": {
            property: this.fillVisualData, // this will be your density property form you geojson
            stops: [
              [0, "#ffd0a6"],
              [10, "#ffd0a6"],
              [100, "#ffaa7f"],
    ...
    calData() {
      for (var i = 0; i < this.allChinaVirus.features.length; i++) {
        this.diagnosis += this.allChinaVirus.features[i]["properties"][
          "new_diagnosis"
        ];
        this.recovery += this.allChinaVirus.features[i]["properties"][
          "new_recovery"
        ];
        this.death += this.allChinaVirus.features[i]["properties"]["new_death"];
      }
    }
  },
  computed: {
    ...mapState({
      allChinaVirus: state => state.virus.allChinaVirus
    }),
    mapSize() {
      let styleObj = {
        width: "100%",
